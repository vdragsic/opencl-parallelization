...
int main(int argc, char *argv[])
{
  ...
  float *A = malloc(sizeof(float) * RANK * RANK);
  float *B = malloc(sizeof(float) * RANK * RANK);
  float *C = malloc(sizeof(float) * RANK * RANK);
  float *D = malloc(sizeof(float) * RANK * RANK);
  ...
  MPI_Bcast (A ,RANK * RANK, MPI_FLOAT, 0, MPI_COMM_WORLD);
  MPI_Bcast (B ,RANK * RANK, MPI_FLOAT, 0, MPI_COMM_WORLD);
  ...
  MPI_Reduce(C, D, RANK*RANK, MPI_FLOAT, MPI_SUM, 0, MPI_COMM_WORLD);
...
/*
-- compile MPI program
mpicc -Wall mpi-mm-float.c -o mpi-mm-float

-- run MPI program
mpirun -np 2 mpi-mm-float

-- run MPI program in gdb
mpirun -np 2 --debug --debugger gdb mpi-mm-float
*/
