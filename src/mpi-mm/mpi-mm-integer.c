...
int main(int argc, char *argv[])
{
  ...
  int *A = malloc(sizeof(int) * RANK * RANK);
  int *B = malloc(sizeof(int) * RANK * RANK);
  int *C = malloc(sizeof(int) * RANK * RANK);
  int *D = malloc(sizeof(int) * RANK * RANK);
  ...
  if (myid == 0)
    {
      for (i = 0; i < RANK*RANK; i++)
	{
	  A[i] = (int)((float)rand() / RAND_MAX * 10);
	  B[i] = (int)((float)rand() / RAND_MAX * 10);
	}
    }
  ...
  MPI_Bcast (A ,RANK * RANK, MPI_INT, 0, MPI_COMM_WORLD);
  MPI_Bcast (B ,RANK * RANK, MPI_INT, 0, MPI_COMM_WORLD);
  ...
  MPI_Reduce(C, D, RANK*RANK, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);
...
/*
-- compile MPI program
mpicc -Wall mpi-mm-int.c -o mpi-mm-int

-- run MPI program
mpirun -np 2 mpi-mm-int

-- run MPI program in gdb
mpirun -np 2 --debug --debugger gdb mpi-mm-int
*/
